...
# ^^^ I commented out the above line, because it was running the code without any arguments. -Andy
...
for size in $sizes; do
    output_custom=$(mpirun --oversubscribe -np $size ./build/main --custom)
    echo "$output_custom"
    latency_custom=`echo "$output_custom" | awk '/Custom Barrier Time:/ {print $4}'`

    output_built_in=$(mpirun --oversubscribe -np $size ./build/main --built-in)
